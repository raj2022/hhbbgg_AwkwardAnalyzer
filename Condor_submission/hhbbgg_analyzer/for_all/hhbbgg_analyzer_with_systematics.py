...
# ---------------- Transient-I/O retry helper ----------------
def _read_parquet_with_retry(inputfile, max_retries=4, delay=5):
    """Open a ParquetFile with retries, to ride out transient EOS I/O
    errors (OSError: [Errno 5] Input/output error) rather than failing
    an entire multi-hour run over a single flaky read.

    Only wraps the metadata-open call (cheap, idempotent) -- NOT
    iter_batches(), since retrying mid-batch-iteration would re-read
    from the start of the file and risk duplicating already-written
    events for this file. A failure during iter_batches() is instead
    allowed to propagate up to main(), which treats the whole file as
    failed and deletes its group's incomplete output (see main()) so a
    plain rerun of the same command reprocesses it cleanly.
    """
    last_exc = None
    for attempt in range(1, max_retries + 1):
        try:
            return pq.ParquetFile(inputfile)
        except OSError as e:
            last_exc = e
            print(f"[WARN] Failed to open {inputfile} (attempt {attempt}/{max_retries}): {e}")
            if attempt < max_retries:
                time.sleep(delay)
    raise last_exc


def _recreate_with_retry(tree_file_path, max_retries=4, delay=5):
    """Open a fresh uproot output file with retries, to ride out transient
    EOS I/O errors on file creation (observed as both Errno 5 and Errno 71
    "Protocol error") -- same rationale as _read_parquet_with_retry, but
    for the write side (uproot.recreate). Only wraps the initial
    recreate() call, not subsequent writes to an already-open file."""
    last_exc = None
    for attempt in range(1, max_retries + 1):
        try:
            return uproot.recreate(tree_file_path)
        except OSError as e:
            last_exc = e
            print(f"[WARN] Failed to create {tree_file_path} (attempt {attempt}/{max_retries}): {e}")
            if attempt < max_retries:
                time.sleep(delay)
    raise last_exc
...
def process_parquet_file(inputfile, cli_year, cli_era, xsec_lumi_cache=None, tree_upfile=None,
    ...
    parquet_file = _read_parquet_with_retry(inputfile)
...
def main():
    ...
    ap.add_argument(
        ...
        help="Base output directory. Output is written to "
             "<outdir>/merged/<tag>/. Defaults to './outputfiles' (relative "
             "to the current working directory) if not given, preserving "
             "the original behavior -- pass this explicitly to write "
             "output somewhere other than the script's own working "
             "directory (e.g. a shared EOS area with more quota/space than "
             "an AFS home volume)."
    )
    ...
    out_base = Path(args.outdir) if args.outdir else Path("outputfiles")
    out_dir = out_base / "merged" / out_tag
    out_dir.mkdir(parents=True, exist_ok=True)
    print(f"[INFO] Output base directory: {out_base}")
    print(f"[INFO] Output directory (this tag): {out_dir}")
    ...
    # Tracks every file that ultimately failed (after retries) across the
    # whole run, purely for the summary printed at the end of main() --
    # does not affect resumability logic itself, which is handled per-group
    # below via group_had_failure / deleting incomplete tree_file_path.
    FAILED_FILES = []
    ...
    if not args.skip_trees:
        for (sample_name, syst_name), group_files in files_by_group.items():
            ...
            tree_upfile = _recreate_with_retry(tree_file_path)
            TREE_CACHE.clear()

            # Tracks whether ANY file in this group failed even after the
            # open-retry logic in _read_parquet_with_retry() -- if so, this
            # group's tree_file_path is deleted below rather than left in
            # place, so a plain rerun of the same command sees it as
            # missing and correctly reprocesses the WHOLE group (simpler
            # and safer than trying to patch in just the missing file's
            ...
            for infile in group_files:
                ...
                try:
                    ...
                    print(f"[FAILED] {infile}: I/O error after retries, skipping this file "
                          f"for now: {e}")
            ...
            if group_had_failure:
                print(f"[WARN] '{sample_name}' / '{syst_name}': one or more files failed -- "
                      f"deleting incomplete output so a rerun of this same command "
                      f"reprocesses this group from scratch: {tree_file_path}")
                tree_file_path.unlink(missing_ok=True)
            else:
        ...
        for infile in inputfiles:
            ...
            try:
                ...
                print(f"[FAILED] {infile}: I/O error after retries, skipping this file: {e}")
                FAILED_FILES.append(infile)
                continue
    ...
    if FAILED_FILES:
        print(f"[SUMMARY] {len(FAILED_FILES)} file(s) failed (after retries) and were "
              f"skipped this run -- their (sample, systematic) group's tree output was "
              f"deleted so a rerun of this exact command will reprocess them:")
        for f in FAILED_FILES:
            print(f"    - {f}")
    else:
        print("[SUMMARY] No files failed this run.")
    print("======================================")

...
assumptions cleanly:
  ...
  - `--split-mc` does NOT exist as a flag on the deployed produce_one_mc.py --
    confirmed directly from its own argparse usage error when this was tried:
    "error: unrecognized arguments: --split-mc". An internal, automatic
    year-based split (config["split_mc"] = True when "2024"/"2025" in year,
    seen in one local copy of update_json_config()) may or may not exist in
    the deployed version -- NOT independently confirmed either way. This
    script does not pass --split-mc at all; if event-ID splitting between
    2024/2025 turns out not to happen automatically, that's a real open
    question worth checking directly (grep update_json_config() for
    "split_mc" on the actual deployed file) before trusting 2024 and 2025
    output are genuinely non-overlapping.
...
NMSSM_Samples = {
    ...
    500: [50, 60, 70, 80, 90, 95, 100, 150, 170, 200, 250, 300, 350],
    # 500: [125], # X500_Y125 excluded: INVALID
...
# --- State handling -- CONCURRENCY-SAFE version ---
# The original version loaded state ONCE at startup and held it in memory for
# the whole run. Confirmed real bug (2026-08-24): running this script from
# multiple lxplus nodes concurrently caused one process's final save_state()
# to overwrite the file with its own stale in-memory snapshot, silently
# erasing dozens of samples the OTHER process had already recorded -- lost
# only recoverable by manually reconciling against real EOS output + condor_q.
#
# Fix: never hold a long-lived in-memory copy for the read/write decisions
# that matter. Every check and every write re-reads fresh from disk right at
# that moment, shrinking the race window from "the whole batch duration"
# (hours) to milliseconds around a single read-modify-write. Not a real lock
# (EOS/AFS file locking is unreliable across nodes anyway, which is exactly
# the situation that caused this) -- a residual, much smaller race still
# exists (two processes could each see "not yet submitted" and both submit
# in the same instant), but that produces at most a harmless duplicate
# submission (matching output filenames just get overwritten), never the
# silent data LOSS this fix is actually for.
def load_state():
...
def mark_failed(state_key):
    current = load_state()
    if state_key not in current["failed"]:
        current["failed"].append(state_key)
    with open(STATE_FILE, "w") as f:
        json.dump(current, f, indent=2)

# Still used for --check/--dry-run reporting and the startup summary, where a
# single snapshot read is fine (those are read-only, informational, and not
# part of the read-modify-write cycle that caused the actual data loss).
state = load_state()
...
to_check = [s for s in samples if not is_already_submitted(s["state_key"])]
...
for sample in samples:
    ...
    # Fresh read right before deciding -- not the stale snapshot from
    # script startup. Catches samples another concurrent instance (a
    # different terminal/node) has submitted since this process started.
    if is_already_submitted(state_key):
        print(f"[SKIP] {state_key} -- already submitted (this run or another concurrent one).")
        continue
    ...
    try:
        ...
        mark_submitted(state_key)   # read-modify-write fresh, not the stale in-memory dict
...
print("\n" + "=" * 60)
final_state = load_state()   # fresh read for the final summary too
year_submitted = [k for k in final_state["submitted"] if k.startswith(args.year)]
year_failed = [k for k in final_state["failed"] if k.startswith(args.year)]

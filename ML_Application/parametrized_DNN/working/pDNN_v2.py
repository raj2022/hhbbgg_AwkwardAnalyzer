...
def df_to_arrays(df: pd.DataFrame, feature_list: Sequence[str]) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
...
def plot_correlation_heatmap(x_te_raw: np.ndarray, feature_list: Sequence[str], cfg: Config = CFG) -> None:
    """Pearson correlation heatmap of physics features (raw scale, excludes mass/y_value)."""
    out_dir = os.path.join(cfg.PLOT_DIR, "FeatureImportance")
    df = pd.DataFrame(x_te_raw, columns=feature_list)
...
def main() -> None:
    ...
    plot_correlation_heatmap(x_te_raw=df_to_arrays(df_te, feature_list)[0], feature_list=feature_list, cfg=cfg)

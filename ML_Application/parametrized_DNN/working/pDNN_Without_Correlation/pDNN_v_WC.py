...
# # =============================================================================
# # pDNN_v2.py
# # Written by: Shivam Raj (CUA, CERN) <CMS X-YH group>
...
# # =============================================================================
# # =============================================================================
# # Fix 1: FEATURES_CORE -- REMOVE the three excluded variables (this is the
# # actual training-input list; this is where the exclusion belongs).
# # =============================================================================
...
# ]
# # Removed vs. your current version: "Res_dijet_mass_DNNreg", "Res_dijet_mass"
# # (from the "Res_dijet_pt", "Res_dijet_mass" line), and "Res_M_X" entirely.


# # =============================================================================
# # Fix 2: RAW_COLUMNS_OF_INTEREST -- UNCOMMENT these three (restore them
# # exactly as they were before). This list controls what gets READ from
# # disk at all, independent of whether it ends up as a training feature --
# # these three need to stay readable so mass_sculpting_validation.py can
# # still find Res_dijet_mass as an observable to test the score against.
# # =============================================================================
...
#     "lead_mvaID", "lead_r9",
#     "sigma_m_over_m", "sublead_r9",
#              # to avoid colliding with the "mass" column used for the signal grid point.
# ]


# # =============================================================================
# # Fix 3: _prepare_raw()'s "extras" -- add these three so they survive
# # being read AND survive the post-read column-keep filter, exactly the
# # same mechanism already used to keep diphoton_mass around despite it
# # not being a FEATURES_CORE training input.
# #
# # Find this line inside _prepare_raw():
# #     extras = [c for c in (CFG.WEIGHT_COL, "diphoton_mass") if c in df.columns]
# # Replace with:
# #     extras = [c for c in (CFG.WEIGHT_COL, "diphoton_mass", "Res_dijet_mass",
# #                           "Res_dijet_mass_DNNreg", "Res_M_X") if c in df.columns]
# # =============================================================================
...
# # Add this constant near the top of pDNN_v_WC.py, alongside the other
# # config constants (CFG.SEED, etc.):
# SENTINEL_THRESHOLD = -900.0
# # HiggsDNA/pDNN-stage variables (e.g. Res_M_X, lead_mvaID) use an
# # unphysical default around -999/-1000 when a reconstruction quantity
# # fails. This is a real value, not NaN, so isna() never catches it: it
# # is neither imputed for its own row NOR excluded from the "observed"
# # pool used to fill in genuinely-missing entries elsewhere -- meaning a
# # sentinel value can be actively RESAMPLED as a fill-in for other rows,
# # spreading the contamination rather than just leaving it isolated in
# # its own row. None of this pipeline's features can legitimately take a
# # value anywhere near -900, so a single threshold safely applies across
# # all of them.
...
#     Sentinel/placeholder values (e.g. ~-999/-1000, used by HiggsDNA-stage
#     variables to mark a failed reconstruction) are converted to NaN
#     BEFORE the missing-value check below, so they are treated as missing
#     data (imputed the same way as genuine NaNs) rather than passing
#     through untouched as extreme real-valued outliers, and rather than
#     being eligible to be resampled as fill-in values for other rows.

#     FIXED: weight_central itself was never checked for NaN/Inf.
#     Confirmed directly (13.1M-event data pull): 13 NaN and 103 Inf
#     weight_central values are present (a real, if tiny, ~0.00088%
#     fraction of events -- likely a scale-factor lookup returning a
#     degenerate value for events at the edge of its calibrated range).
#     train_one_epoch() computes wb.mean() per BATCH; a single NaN/Inf
#     weight anywhere in a batch poisons that batch's mean and therefore
#     the loss for every event in it. Since BATCH_SIZE_TRAIN=128 and
#     training reshuffles every epoch, essentially every batch across
#     every epoch eventually contains one of these 116 events, which
#     explains TrainLoss being NaN from epoch 1 onward while ValAUC (which
#     never touches sample weights) looked completely normal.
#     Non-finite weights are zeroed out here -- not dropped as rows, so
#     array shapes/indexing stay untouched -- meaning these 116 events
#     contribute exactly zero to the loss and to the batch weight-mean,
...
#         x_df[c] = x_df[c].where(x_df[c] > SENTINEL_THRESHOLD, np.nan)
...
#     # NEW: sanitize the weight column -- zero out any NaN/Inf entries so
#     # they can never poison a batch's weight mean, without changing
#     # array length/shape. ALSO clip negative weights to zero: confirmed
#     # directly (938 events, all tiny floating-point-noise magnitudes like
#     # -2.5e-08, not genuine large negative-weight MC events) that these
#     # break sklearn's roc_curve()/auc() downstream, since FPR/TPR are
#     # cumulative sums of weights and a cumulative sum is only guaranteed
#     # monotonic if every weight is non-negative -- a single negative
#     # weight can make the cumulative sum tick backwards at some
#     # threshold, which is exactly what "x is neither increasing nor
#     # decreasing" means.
#     w = df[CFG.WEIGHT_COL].astype("float32").values
#     n_bad_weights = int((~np.isfinite(w)).sum())
#     n_negative_weights = int((np.isfinite(w) & (w < 0)).sum())
#     if n_bad_weights:
#         print(f"[WARN] df_to_arrays: {n_bad_weights} non-finite {CFG.WEIGHT_COL} "
#               f"value(s) found and zeroed out (contribute nothing to loss).")
#     if n_negative_weights:
#         print(f"[WARN] df_to_arrays: {n_negative_weights} negative {CFG.WEIGHT_COL} "
#               f"value(s) found (tiny floating-point-noise magnitudes, not genuine "
#               f"negative-weight MC events) and clipped to zero -- required for "
#               f"roc_curve()/auc() downstream, which assume non-negative weights "
#               f"for their cumulative-sum FPR/TPR computation.")
#     w = np.where(np.isfinite(w) & (w >= 0), w, 0.0).astype("float32")
...
#     FIXED: the previous version called pd.read_parquet(file_path,
#     columns=None) just to inspect the column names -- that reads the
#     ENTIRE file (every column, every row) into memory before the real,
#     column-subset read even happens, doubling I/O and peak memory for
#     every single file. pq.read_schema() reads only the file's schema
#     (column names/types), not the data itself, so this now reads each
#     file exactly once, with only the columns actually needed.
 
#     Falls back to reading the entire file if the column-subset read
#     fails for any reason (e.g. schema surprises).
#     """
#     try:
#         cols = pq.read_schema(file_path).names
...
class Config:
    ...
    # UPDATED (2026-09-11): the mY floor was revised from 90 GeV to 70 GeV
    # for this analysis, based on the turn-on/mZ-resolution study documented
    # in the AN (Section "Signal Mass Grid Boundary at Low mY"): no
    # trigger/reconstruction efficiency turn-on was found down to mY = 50
    # GeV, and mY = 90/95/100 GeV -- despite a real, localized dijet-mass
    # resolution loss near mZ -- are RETAINED rather than excluded, since a
    # dedicated data-driven search did not confirm a Z->bb origin for that
    # degradation.
    #
    # MASS_POINTS/Y_VALUES together form a rectangular cross-product, but
    # the real signal grid is NOT rectangular -- each mX has its own list
    # of valid mY values. load_signal()'s nested loop tries every (X, Y)
    # combination in the cross-product below and silently skips whatever
    # isn't found on disk via the existing os.path.exists() check -- this
    # is expected and NOT an error; a found/skipped/tried summary is
    # printed at the end of load_signal() so this stays visible rather
    # than fully silent.
    #
    # CONFIRMED EXCEPTION (2026-09-11, direct EOS scan via
    # check_signal_grid.py): mX = 1000 is NOT part of the mY = 70/80
    # extension -- production for mX = 1000 only exists down to mY = 90,
    # exactly as in the previous (pre-revision) grid. Trying
    # X1000_Y70/X1000_Y80 is therefore expected to always miss and get
    # skipped; this is a genuine production gap, not a bug in this script
    # or in the mY-floor revision itself.
    #
    # Two further gaps were also confirmed directly on disk and are NOT
    # bugs either (same "non-rectangular grid, skip what's missing" logic
    # already handles them): mX = 450 has no mY = 125 point, and mX = 700
    # has no mY = 60 or mY = 95 point.
    #
    # Because of the above, do NOT rely on a hardcoded expected-point-count
    # comment here (the previous version of this file quoted "196 points,
    # X >= 300, Y >= 90" -- that count is now stale on both the mY floor
    # and the exact total). Trust load_signal()'s printed found/tried
    # summary at runtime instead; see check_missing_masses.py for a
    # standalone cross-check against the full expected combination list.
    MASS_POINTS: Tuple[int, ...] = (
        300, 320, 350, 400, 450, 500, 550, 600, 650, 700, 750, 800, 850, 900, 950, 1000,
    )
    Y_VALUES: Tuple[int, ...] = (
        70, 80, 90, 95, 100, 125, 150, 170, 200, 250, 300, 350, 400, 450, 500, 550, 600, 650, 700, 800,
    )
    WEIGHT_COL: str = "weight"
...
# Removed vs. the earlier version: "Res_dijet_mass_DNNreg", "Res_dijet_mass"
# (from the "Res_dijet_pt", "Res_dijet_mass" line), and "Res_M_X" entirely --
# see the mass-sculpting-risk note further down this file for why.
...
def df_to_arrays(df: pd.DataFrame, feature_list: Sequence[str], seed: int = CFG.SEED) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    weight_central itself is checked for NaN/Inf. Confirmed directly
    (13.1M-event data pull): 13 NaN and 103 Inf weight_central values are
    present (a real, if tiny, ~0.00088% fraction of events -- likely a
    scale-factor lookup returning a degenerate value for events at the
    edge of its calibrated range). train_one_epoch() computes wb.mean()
    per BATCH; a single NaN/Inf weight anywhere in a batch poisons that
    batch's mean and therefore the loss for every event in it. Non-finite
    weights are zeroed out here -- not dropped as rows, so array
    shapes/indexing stay untouched -- meaning these events contribute
    exactly zero to the loss and to the batch weight-mean, rather than
    corrupting either.
    """
    x_df = df[list(feature_list)].copy()
    ...
    # Sanitize the weight column -- zero out any NaN/Inf entries so
    # they can never poison a batch's weight mean, without changing
    # array length/shape. Also clip negative weights to zero: confirmed
    # directly (938 events, all tiny floating-point-noise magnitudes like
    # -2.5e-08, not genuine large negative-weight MC events) that these
    # break sklearn's roc_curve()/auc() downstream, since FPR/TPR are
    # cumulative sums of weights and a cumulative sum is only guaranteed
    # monotonic if every weight is non-negative.
    w = df[CFG.WEIGHT_COL].astype("float32").values
    n_bad_weights = int((~np.isfinite(w)).sum())
    n_negative_weights = int((np.isfinite(w) & (w < 0)).sum())
    if n_bad_weights:
...
def predict_batched(model: nn.Module, x_tensor: torch.Tensor, device: torch.device,
    ...
    Logits are explicitly upcast to float32 and sigmoid is applied OUTSIDE
    the autocast context below, so autocast still speeds up the forward
    pass itself, but the probability actually used downstream is computed
    at full precision (avoids a visibly quantized "comb" pattern in the
    score distribution's tail under float16).
    """
    model.eval()
    n = x_tensor.shape[0]
    out = np.empty(n, dtype=np.float32)
...
def _read_parquet_slim(file_path: str) -> pd.DataFrame:
    ...
    pq.read_schema() reads only the file's schema (column names/types),
    not the data itself, so this reads each file exactly once, with only
    the columns actually needed. Falls back to reading the entire file if
    the column-subset read fails for any reason (e.g. schema surprises).
    """
    try:
        cols = pq.read_schema(file_path).names
        subset = [c for c in (set(RAW_COLUMNS_OF_INTEREST) | {CFG.WEIGHT_COL}) if c in cols]
...
def load_signal(cfg: Config = CFG) -> pd.DataFrame:
    ...
    MASS_POINTS/Y_VALUES together form a rectangular cross-product, but
    the real signal grid is NOT rectangular (see the comment beside those
    fields in Config, including the confirmed mX=1000 mY-floor exception
    and the mX=450/mX=700 point gaps) -- most (X, Y) combinations tried
    here are expected to not exist on disk and are silently skipped, not
    an error. A found/skipped/found-vs-expected summary is printed at the
    end so this is visible rather than fully silent; missing optional
    columns in a found file are still handled gracefully by
    ...
    print(f"[INFO] load_signal: found {n_found}/{n_tried} (X,Y) combinations tried "
          f"({n_skipped_missing} did not exist on disk -- expected, given the "
          f"non-rectangular grid; known exceptions include mX=1000 (mY floor "
          f"stays at 90, not 70), mX=450 (no mY=125), and mX=700 (no mY=60 "
          f"or mY=95). See check_missing_masses.py to cross-check against the "
          f"full expected combination list.")
    signal_df = pd.concat(rows, ignore_index=True) if rows else pd.DataFrame()
    if signal_df.empty:
        raise RuntimeError("No signal samples could be loaded.")
    return signal_df
